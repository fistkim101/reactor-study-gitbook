...
![request per thread](<.gitbook/assets/스크린샷 2022-05-22 오후 8.17.34 (4).png>)
...
![block이 발생하면 스레드는 논다](<.gitbook/assets/스크린샷 2022-05-22 오후 8.18.11 (1).png>)
...
![Event Loop 방식](<.gitbook/assets/스크린샷 2022-05-22 오후 8.29.05.png>)
...
![](<.gitbook/assets/스크린샷 2022-05-22 오후 8.39.07.png>)
...
![](<.gitbook/assets/스크린샷 2022-05-22 오후 9.10.12.png>)
...
![https://developer.ibm.com/articles/l-async/](<.gitbook/assets/image (7).png>)
...
### 영상이랑 똑같이 캐시든 뭐든 레포지토리 만들어서 똑같은 실수 해보고 성능테스트 해보기. 스레드 변경되는 것도 살펴보기. 결국 서브스크라이브 한 스레드가 퍼블리쉬까지 하는 부분(=스레드 따로 할당해주지 않은 부분)은 사실 map이든 flapMap이든 다를바가 없고, 디비 조회나 웹클라이언트 로직과 같은 부분에서 publisher 로 호출해줘야 보내고 웨이팅 안하고 해당 스레드가 다른 일도 처리하는 것. 그렇다면 기다린 응답이 왔을때 과연 일을 하던 그 스레드가 하는지, 다른 스레드가 하는지는확인이 필요하다) => 핵심은 외부 서버 응답, 디비에 대한 응답 대기시 블로킹되지 않고 다른 일을 처리하도록 하는게 중요하고 그때 publisher 로 처리해두는 것이 중요한 것이었군.
